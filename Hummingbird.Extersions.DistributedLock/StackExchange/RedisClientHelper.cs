...
namespace Hummingbird.Extersions.DistributedLock.StackExchangeImplement
{
    ...
    class RedisClientHelper
    {
        ...
        public T ListRightPopLeftPush<T>(string source, string destination)
        {
            ...
            var value = Do(db => db.ListRightPopLeftPush(source, destination));
        ...
        {
            key = AddSysCustomKey(key);
            return Do(db => db.LockRelease(key, lockValue));
        }
        ...
        public void Subscribe<T>(string subChannel, Action<RedisChannel, T> handler = null)
        {
            ...
            {
                Console.WriteLine(subChannel + " 订阅收到消息：" + message.ToString());
                ...
                {
                    var value = ConvertObj<T>(message);
        ...
        {
            var prefixKey = CustomKey ?? KeyPrefix;
