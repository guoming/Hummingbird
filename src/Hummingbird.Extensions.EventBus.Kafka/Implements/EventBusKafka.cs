                            ...
                            try
                            {
                                ...
                                else
                                {
                                    _logger.LogInformation(
                                        $"Consumed message '{ea.Value}' at: '{ea.TopicPartitionOffset}'.");
                                }
                                ...
                                using (var tracer = new Hummingbird.Extensions.Tracing.Tracer("AMQP Received", TraceId))
                                {
                                    ...
                                    if (ea.Headers != null)
                                    {
                                        try
                                        {
                                            long.TryParse(
                                                System.Text.Encoding.UTF8.GetString(
                                                    ea.Headers.GetLastBytes("x-eventId")), out EventId);
                                        }
                                        catch
                                        {
                                        }
                                        ...
                                        try
                                        {
                                            TraceId = System.Text.Encoding.UTF8.GetString(
                                                ea.Headers.GetLastBytes("x-traceId"));
                                        }
                                        catch
                                        {
                                        }
                                    ...
                                    try
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            eventResponse.Body =
                                                JsonConvert.DeserializeObject<TD>(eventResponse.BodySource);
                                            ...
                                            {
                                                eventResponse.Headers.Add(key.Key,
                                                    Encoding.UTF8.GetString(key.GetValueBytes()));
                                            }
                                        ...
                                        using (var tracerExecuteAsync =
                                               new Hummingbird.Extensions.Tracing.Tracer("AMQP Execute"))
                                        {
                                            ...
                                            finally
                                            {
                                                if (!handlerSuccess)
                                                {
                                                    ...
                                                    try
                                                    {
                                                        ...
                                                        {
                                                            requeue = await _subscribeNackHandler((
                                                                new EventResponse[] { eventResponse },
                                                                handlerException));
                                                        }
                                                    }
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        _logger.LogInformation(
                                                            $"kafk offset seek,topic={routeKey} partation={ea.TopicPartition.Partition}offset={ea.TopicPartitionOffset.Offset.Value}");
                                                    }
                                                }
                                            }
                                        }
                            ...
                            try
                            {
                                ...
                                try
                                {
                                    ...
                                    for (int j = 0; j < eas.Length; j++)
                                    {
                                        ...
                                        using (var tracer =
                                               new Hummingbird.Extensions.Tracing.Tracer("AMQP Received", TraceId))
                                        {
                                            ...
                                            try
                                            {
                                                ...
                                                Messages[j].Body =
                                                    JsonConvert.DeserializeObject<TD>(Messages[j].BodySource);
                                                ...
                                                foreach (var key in ea.Headers)
                                                {
                                                    Messages[j].Headers.Add(key.Key,
                                                        Encoding.UTF8.GetString(key.GetValueBytes()));
                                                }
                                    ...
                                    if (Messages != null && Messages.Any())
                                    {
                                               ...
                                               new Hummingbird.Extensions.Tracing.Tracer("AMQP Execute"))
                                        {
                                            ...
                                            handlerSuccess = await _receiverPolicy.ExecuteAsync(
                                                async (handlerCancellationToken) =>
                                                {
                                                    return await eventAction.Handle(
                                                        Messages.Select(a => (TD)a.Body).ToArray(),
                                                        Messages.Select(a => (Dictionary<string, object>)a.Headers)
                                                            .ToArray(), handlerCancellationToken);
                                                }, cancellationToken);
                                        ...
                                        else
                                        {
                                            if (eas.Length > 0)
                                            {
                                                foreach (var group in eas.GroupBy(a => a.Partition))
                                                {
                                                    ...
                                                    Console.WriteLine(
                                                        $"kafk offset seek,topic={routeKey}                       partation={group.FirstOrDefault().TopicPartition.Partition}offset={group.FirstOrDefault().TopicPartitionOffset.Offset.Value}");
