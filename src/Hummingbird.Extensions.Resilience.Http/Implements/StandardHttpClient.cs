       ...
       HttpMessageHandler httpMessageHandler)
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            uri = await _httpUrlResolver.Resolve(uri);
                ...
                using (var tracer =
                       new Hummingbird.Extensions.Tracing.Tracer($"HTTP {HttpMethod.Post.Method.ToUpper()}"))
                {
                    tracer.SetComponent(_compomentName);
                    tracer.SetTag("http.url", uri);
                    ...
                    if (dictionary != null && dictionary.ContainsKey("x-masking") &&
                        (dictionary["x-masking"] == "all" || dictionary["x-masking"] == "request"))
                    {
                        //日志脱敏                           
                    }
                    else
                    ...
                    if (dictionary != null && dictionary.ContainsKey("x-masking") &&
                        (dictionary["x-masking"] == "all" || dictionary["x-masking"] == "response"))
                    {
                        //日志脱敏不记录
                    }
                    else
